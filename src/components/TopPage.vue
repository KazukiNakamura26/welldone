...
export default {
  ...
  data() {
    return {
      ...
      messages: [
        { title: '案件A', message: 'ここに何かメッセージが入ります。', tolink: 'project'},
        { title: 'テクノモバイル', message: 'ここに何かメッセージが入ります。', tolink: 'project'}
      ]
    }
  },
  methods: {
...
// プロジェクト一覧を取得
    ...
    userRef.get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          projects.push({ title: doc.data().name, tolink: '/project/' + doc.id})
        });
...
// メッセージ一覧を取得
    const messageRef = firebase.firestore().collection("praise").where('praise_user_id', '==', userId)
